  ...
  return (
    <div className="relative min-h-screen transition-colors duration-300 dark:bg-[#040507] bg-[#F7F7F8] text-slate-900 dark:text-slate-100 font-sans selection:bg-amber-500/20 selection:text-amber-500 px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-10">
        ...
        <div className="dark:bg-[#08090C]/80 bg-white/80 border dark:border-white/[0.07] border-black/[0.08] backdrop-blur-xl p-4 sm:p-5 rounded-3xl shadow-xl space-y-4 md:space-y-0 md:flex md:items-center md:justify-between gap-4">
          ...
          <div className="relative flex-1 max-w-lg">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-2.5 dark:bg-black/40 bg-slate-50 border dark:border-white/10 border-black/10 rounded-full text-slate-900 dark:text-white placeholder-slate-400 text-xs focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 font-sans transition-all"
            />
          </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-semibold font-mono tracking-wider transition-all border ${
                  selectedCategory === cat.id
                    ? 'dark:bg-white dark:text-slate-950 bg-slate-900 text-white shadow-md'
                    : 'dark:bg-white/5 bg-slate-100 text-slate-700 dark:text-slate-400 border-black/10 dark:border-white/10 hover:border-amber-500/50 hover:text-amber-500'
                }`}
              >
                {cat.label}
              </button>
        ...
        {filteredMentors.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredMentors.map((mentor) => (
              <div
                key={mentor.id}
                className="dark:bg-[#08090C]/80 bg-white/90 border dark:border-white/[0.07] border-black/[0.08] shadow-xl rounded-3xl p-6 hover:border-amber-500/30 transition-all space-y-5 flex flex-col justify-between group"
              >
                <div className="space-y-4">
                  {/* Peer Header */}
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-extrabold text-xs flex items-center justify-center font-mono">
                        {mentor.avatar}
                      </div>
                      <div>
                    ...
                    <span className="text-[10px] font-mono px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-300 border border-amber-500/20 font-bold">
                      ★ {mentor.rating}
                    </span>
                  </div>
                ...
                <Link
                  to="/register"
                  className="w-full py-2.5 rounded-full font-semibold text-xs dark:bg-white dark:text-slate-950 bg-slate-900 text-white hover:opacity-90 transition-all flex items-center justify-center gap-1.5 shadow-md mt-4 font-mono uppercase tracking-wider"
                >
